...
class UserMcpScreen extends StatefulWidget {
  ...
  @override
  State<UserMcpScreen> createState() => _UserMcpScreenState();
}

class _UserMcpScreenState extends State<UserMcpScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _repo.listUserMcpServers();
      if (!mounted) {
        return;
      }

      setState(() {
        _servers = list.where((s) => !s.isGlobal).toList();
        _loading = false;
      });
    } catch (e) {
      if (!mounted) {
  ...
  Future<void> _delete(McpServerEntity s) async {
    ...
    try {
      await _repo.deleteUserMcpServer(s.id);
      if (!mounted) {
        return;
      }
  ...
  Future<void> _openEditor({McpServerEntity? existing}) async {
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          title: Text(
            isNew ? 'Новый личный MCP-сервер' : 'Редактировать MCP #$editId',
          ),
          content: SizedBox(
            width: 520,
    ...
    try {
      if (isNew) {
        await _repo.createUserMcpServer(entity);
      } else {
        await _repo.updateUserMcpServer(entity);
      }
